...
from DES_Python.rngs import selectStream, random, plantSeeds
...
class track:
  ...
  def __str__(self):
    return f"Area: node={self.node}, queue={self.queue}, service={self.service}"

class time:
  def __init__(self):
    self.current = 0.0                  # current time                        */
    self.next = 0.0                     # next (most imminent) event time     */
    self.last = [0.0] * NUM_BLOCKS      # last arrival time                   */

  def __str__(self):
    return f"Time: current={self.current}, next={self.next}, last={self.last}"
