...
from DES_Python.rngs import plantSeeds, selectStream, random
...
from inout import append_to_json
...
class NodeMultiServer(Node):
    ...
    def get_available_server(self, time):
        sid = next(i for i in range(len(self.subcompletions)) if
                   self.subcompletions[i] <= time or self.subcompletions[i] == INFINITY) #FIXME inifinity
...
class Simulation:
    ...
    def schedule_new_event(self, event_type: EventType, time: float = None, request: Request = None, node_id: int = None,
    ...
    def get_system_arrival(self, arrivalTemp):
        self.arrivals_ctr += 1 # todo dividere gli streams
        arrivalTemp += (
    ...
    def GetService(self,node_type: NodeType, is_catC: bool = None):
        if node_type == NodeType.EDGE:
            ...
            if self.exec_mode != ExecutionMode.VERIFY:
                stddev=0.025
                if not is_catC: return TruncatedNormal(0.5, stddev, 0.48, 3*stddev)
                else:           return TruncatedNormal(0.1, stddev, 0.08, 0.15)
            else:
    ...
    def run(self):
        ...
        for replica in range(self.replicas_no):
            for seed_iter in range(1):
                current_seed=self.seed#seeds[seed_iter]
                ...
                self.nodes = [EdgeNode() for _ in range(self.edge_no)] + [CloudNode(self.edge_no) for _ in range(NUM_CLOUD)] + [
                ...
                while (self.exec_mode in [ExecutionMode.VERIFY, ExecutionMode.INFINITE_HORIZON] and len(self.samplepoints)<N) or\
                    (self.exec_mode not in [ExecutionMode.VERIFY, ExecutionMode.INFINITE_HORIZON]  and len(self.event_list) > 0 and self.t.current < self.STOP):  # or esistono_richieste_nei_nodi(nodes):
                    ...
                    if event.event_type in [EventType.ARRIVAL_EDGE, EventType.FEEDBACK_FROM_CLOUD]:
                        if not(event.event_type==EventType.FEEDBACK_FROM_CLOUD and self.exec_mode==ExecutionMode.VERIFY):
                            ...
                            if self.nodes[selected_node].number == 1:
                                ...
                                to_be_scheduled = self.schedule_new_event(EventType.COMPLETION_EDGE, self.t.current + ts, event.request,
                                                                     selected_node)
                    ...
                    # processa un completamento ai nodi edge
                        ...
                        if event.request.category is None:
                            ...
                            to_be_scheduled = self.schedule_new_event(EventType.ARRIVAL_COORDINATOR, request=event.request)
                            pass
                        ...
                        if self.nodes[completed_node].number > 0:
                            ...
                            ts = 0 if event.request.category == RequestCategory.UNKNOWN else self.GetService(NodeType.EDGE,
                        ...
                        # if nodes[selected_node].number == 1:
                        if self.nodes[selected_node].number <= self.nodes[selected_node].available_servers:
                            ...
                            if sid is None:
                                print("errore sid")
                                exit(1)
                            to_be_scheduled = self.schedule_new_event(EventType.COMPLETION_CLOUD, self.t.current + ts, event.request,
                                                                 selected_node,
                                                                 server_id=sid)
                    ...
                    # processa un completamento ai nodi cloud
                        ...
                        if len(self.nodes[completed_node].reqeusts) > 0:
                            ...
                            # print("schedulo completamento (avanzando la coda) per", t.current + ts)
                        ...
                        nodetype_filter = self.select_node_dest_type(event.event_type)
                        selected_node = self.choose_minimum_load_node(nodetype_filter, event.request.category.is_priority())
                        # accoda il job
                        # nodes[selected_node].push_request(event.request)
                        self.nodes[selected_node].number += 1
                        ...
                        if self.is_node_idle(selected_node):
                            if len(self.nodes[selected_node].reqs_high) == 1:
                                # schedula completamento (== esegui)
                                ts = self.GetService(nodetype_filter)
                                to_be_scheduled = self.schedule_new_event(EventType.COMPLETION_COORDINATOR, self.t.current + ts,
                                                                     event.request,
                                                                     selected_node)
                    ...
                    # processa un completamento ai nodi cloud
                        ...
                        # print("per un evento di categoria", event.request.category.name)
                        if event.request.category.is_priority():
                            ...
                            self.totresptimes["prio"]["value"]= \
                                (self.totresptimes["prio"]["value"]*self.totresptimes["prio"]["index"]+tot_resp_time)/(self.totresptimes["prio"]["index"]+1)
                            self.totresptimes["prio"]["index"]+=1
                        else:
                            self.nodes[completed_node].reqs_low.popleft()
                        tot_resp_time = self.t.current - event.request.generation_time
                        self.totresptimes["all"]["value"]= \
                            (self.totresptimes["all"]["value"]*self.totresptimes["all"]["index"]+tot_resp_time)/(self.totresptimes["all"]["index"]+1)
                        self.totresptimes["all"]["index"]+=1
                        self.totresptimes[event.request.category].append(tot_resp_time)

                        self.totresptimes["double_processing=" + str(event.request.double_processing)].append(tot_resp_time)
                        self.totresptimes[
                        ...
                        print("CAMBIO FASCIA ORARIA ->", fascia.name, ", lambda passa ora a", 1 / expparam_fasciaoraria,
                              "rich/s")
                    ...
                    # campionamento
                    stime=SAMPLING_TIME_INF if self.exec_mode in [ExecutionMode.VERIFY, ExecutionMode.INFINITE_HORIZON] else SAMPLING_TIME_FIN
                    ...
                    if len(self.samplepoints)<=sample_slot:
                        ...
                        dividendo={NodeType.EDGE:1, NodeType.CLOUD:self.edge_no, NodeType.COORDINATOR:self.coord_no}
                        point=StatsPoint(
                        ...
                        out = arrivals_ctr_live / elapsed #sampletime fixme
                        if sample_slot %2000==0:
                            print("- sample n.", sample_slot, seconds_to_hhmm(self.t.current))
                        out = 0
                        arrivals_ctr_live = 0
                        sampleno = 0
                ...
                if self.exec_mode == ExecutionMode.VERIFY:
                    ...
                    for tt in NodeType:
                        ...
                        if index > 0:
                            ...
                            print(f"   Average # in the queue .. = 0.00")
                            print(f"   Utilization ............. = 0.00")
                    print("CATEGORIZZAZIONI TOTALI:", sum(pty), "ARRIVI TOTALI:", arrivals_ctr)  # , "EVENTCTR:", event_ctr)
                ...
                print("")
                print("ob. 2: TdR per richieste dangerous/suspect sia < 1.5 s, indipendentemente da ev. doppio passaggio")
                """
                print("\tTdR dangerous:\t", np.average(self.totresptimes[RequestCategory.DANGEROUS]), np.average(self.totresptimes[
                                                                                                                RequestCategory.DANGEROUS]) <= 1.5)
                print("\tTdR suspect:\t", np.average(self.totresptimes[RequestCategory.SUSPECT]), np.average(self.totresptimes[
                                                                                                            RequestCategory.SUSPECT]) <= 1.5)
                print("\tTdR regular:\t", np.average(self.totresptimes[RequestCategory.REGULAR]), np.average(self.totresptimes[
                                                                                                            RequestCategory.REGULAR]) <= 1.5,
                      "(solo per confronto)")
                print("\tTdR poorqual:\t", np.average(self.totresptimes[RequestCategory.POOR_QUALITY]), np.average(self.totresptimes[
                                                                                                                  RequestCategory.POOR_QUALITY]) <= 1.5,
                      "(solo per confronto)")
                """
                ...
                if self.exec_mode==[ExecutionMode.VERIFY, ExecutionMode.INFINITE_HORIZON]:
